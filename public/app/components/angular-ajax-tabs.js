...
ngAJAXTabs.directive('tabs', function () {
  return {
    ...
    controller: function($scope, $element) {
      ...
      $scope.select = function (pane, $index, $event) {
        var p;
        if ($event && arrayContains($event.srcElement.classList, 'close-tab')) {
          pane.$parent.pane.includedInTabView = false;
      ...
      '<div class="tabbable">' +
        '<ul class="nav nav-tabs">' +
          '<li ng-repeat="pane in panes" ng-class="{active:pane.selected}">'+
            '<a href="" ng-click="select(pane, $index, $event)">{{pane.tabTitle}} <i class="fa fa-times close-tab"></i></a>' +
          '</li>' +
        '</ul>' +
